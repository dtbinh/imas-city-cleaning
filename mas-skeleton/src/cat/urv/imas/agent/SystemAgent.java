...
import static cat.urv.imas.agent.AgentType.HARVESTER;
import static cat.urv.imas.agent.AgentType.SCOUT;
...
public class SystemAgent extends ImasAgent {
    ...
    protected void setup() {
        ...
        ContainerController containterController = this.getContainerController();
        AgentController agentController;
        // Create agents
        try {
            agentController = containterController.createNewAgent("Coordinator", "cat.urv.imas.agent.CoordinatorAgent", null);
            agentController.start();
            agentController = containterController.createNewAgent("HarvesterCoordinator", "cat.urv.imas.agent.HarvesterCoordinator", null);
            agentController.start();
            agentController = containterController.createNewAgent("ScoutCoordinator", "cat.urv.imas.agent.ScoutCoordinator", null);
            agentController.start();
            ...
            for (AgentType agentType : agents.keySet()) {
                ...
                if(agentType == HARVESTER) {
                    ...
                    Logger.getLogger(SystemAgent.class.getName()).log(Level.SEVERE, null, "Wrong agent type");
                }

                List<Cell> agentCells = agents.get(agentType);
                int i = 0;
                for (Cell c : agentCells) {
                    Object [] arguments = new Object[]{c, null};
                    if (agentType.equals(AgentType.HARVESTER)) {
                        arguments[1] = garbageTypes[i];
                    }
                    agentController = containterController.createNewAgent(agentType.getShortString() + "_" + i, className, arguments);
                    agentController.start();
                    i++;
                }
            }
